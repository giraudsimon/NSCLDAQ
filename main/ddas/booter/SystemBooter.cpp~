...
void SystemBooter::boot(Configuration &config, BootType type)
{
    ...
    int retval = Pixie16InitSystem(NumModules, config.getSlotMap().data(), m_offlineMode);
    if(retval < 0) {
...
SystemBooter::bootModuleByIndex(int modIndex, Configuration& m_config, BootType type)
{
    ...
    // daqdev/DDAS#106 - modified to get the per module firmware configuration
    //                   wich will default to the global config if not
    //                   specified
    ...
    // daqdev/DDAS#106  - modified as above to get a per module
    //                    setfile:
 ...
 */
void SystemBooter::setOfflineMode(unsigned short mode)
{
    m_offlineMode = mode;
}

/*!
 * \return the offline mode state
 */
unsigned short SystemBooter::getOfflineMode() const
{
