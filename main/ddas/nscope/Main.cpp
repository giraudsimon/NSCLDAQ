...
#define MADE "nscope v3.0 7/16/2019"
...
Main::Main(const TGWindow * p) 
  :
  pulseshape(0), about(0), energyfilter(0), analogsignal(0), baseline(0),
  csra(0), modcsrb(0), trigconfig(0), triggerfilter(0), cfd(0), tau(0),
  expertmod(0), scopedt(0), histogram(0), maxevent(0), 
  mult_dialogue(0), timing_dialogue(0),
  StateMsgFold1(0), analyzeB(0), acquireB(0), saveB(0),
  bmultdisplay(0), bfastdisplay(0), fStatusBar(0),
  numericMod(0), numericCh(0),
  ftrace_values(0), ftrace(0), fHpx_wave(0), fhisto(0), ffastfilt(0),
  fenerfilt(0), taufitHist(0), dCanvasF1(0),
  selected(0), MenuFile(0), MenuSetup(0), MenuExpert(0), MenuScope(0)
...
void Main::CreateMenuBar()
{
  ...
  MenuSetup->AddEntry("&QDC Lengths", QDCLENGTHS);
  ...
  MenuScope = new TGPopupMenu (fClient->GetRoot ());
	...
	case kCM_MENU:
	  switch (parm1)
		...
		if (RUN_IN_PROGRESS  != 0) {
		  messageBox("Data taking is active and must first be stopped");
		} else {
		  ...
		  if(detector->IsBooted()){
		    ...
		    // If its booted we can enable stuff too:
	    ...
	    case FINDTAU:
	      {
		int retval = FindTau(moduleNr, channelNr, decayconstant, decayconstantfit);
		...
		*decayconstant = 0;
	    ...
	    case CFDS:
	      ...
	      //the 500 MSPS modules do not pay attention to CFD parameters warn and tell the UI
	      // to ghost all but the threshold
	      //
	    ...
	    case QDCLENGTHS:
	      if (RUN_IN_PROGRESS != 0) {
		messageBox("Data Taking is in progress and first must be stopped");
	      } else {
		auto p = new QDCLen(fClient->GetRoot(), 0);
		p->setModule(moduleNr);
		p->load_info(moduleNr);
	      }
	      break;
	    default:
	      break;
...
#endif		    
		    retval = Pixie16ReadStatisticsFromModule(Statistics.data(),moduleNr);
		    if(retval < 0) cout << "failed to read statistics, mod " 
					<< moduleNr << endl;
		    double ICR, OCR, livetime, realtime;
		    for(int z=0;z<16;z++){
		      ICR = Pixie16ComputeInputCountRate(Statistics.data(),moduleNr,z);
		      OCR = Pixie16ComputeOutputCountRate(Statistics.data(),moduleNr,z);
		      livetime = Pixie16ComputeLiveTime(Statistics.data(),moduleNr,z);
		      realtime = Pixie16ComputeRealTime(Statistics.data(),moduleNr);
		     ...
		     unsigned short ChanNum)
{
  ...
  for (unsigned long i = 0; i < size; i++){
	...
	sum2 = sum2 + trace[a];
      ...
      auto kDC = baseline*(a0*(int)enerrise + ag*(int)enerflat + a1*(int)enerrise);
      // if(i == size/2) {
      // 	std::cout << sum2 << " " << sumg << " " << sum1 << " " << kDC << std::endl;
      // } 
      
      enerfilter[i] = a1*sum1 + ag*sumg + a0*sum2 - kDC;     
    }
		   ...
		   double *Tau, double *TauFit) 
{
  ...
  unsigned short trace[MAX_ADC_TRACE_LEN];
  unsigned int traceSize = MAX_ADC_TRACE_LEN;
  int retval;
  ...
  double trigThresh; // Trigger filter threshold
  double tauArray[MAX_ADC_TRACE_LEN]; // Reasonable tau values from analyzed peaks
  double tauArrayFit[MAX_ADC_TRACE_LEN]; // Reasonable tau values from fits
  double tauBins[MAX_ADC_TRACE_LEN]; // Array of tau intervals for binning
  double tauBinsFit[MAX_ADC_TRACE_LEN];
  double tauBinCounts[MAX_ADC_TRACE_LEN]; // Counts in tau bins
  double tauBinCountsFit[MAX_ADC_TRACE_LEN];
  ...
  while (pulseCounter < 200 && traceCounter < 5000) {
    ...
    /* Read a trace */
    if ((retval = Pixie16ReadSglChanADCTrace(trace, MAX_ADC_TRACE_LEN, 
					     ModNum, ChanNum)) < 0) {
...
/**
 * disableSetupMenu
 *    Disables all the commands in the setup menu.
 *    On reason to do this is that the modules are not booted and therefore
 *    any operation the modifies or queries the module will segfault.
 *
 * Implicit inputs:
 *     MenuSetup - pointer to the menu.
 *     SetupMenuCommands - array of commands (ids) to disable.
 */
void
Main::disableSetupMenu()
{
  unsigned nCommands = sizeof(SetupMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuSetup->DisableEntry(SetupMenuCommands[i]);
  }
}
/**
 * enableSetupMenu
 *    Enable the menu entries in the setup menu.
 *    See disableSetupMenu...this is very similar.
 */
void
Main::enableSetupMenu() {
  unsigned nCommands = sizeof(SetupMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuSetup->EnableEntry(SetupMenuCommands[i]);
  }
  
}
/**
 * disable/enableExpertMenu
 *     Turns off/on all the commands in the expert menu. . This is
 *     similar to enable/disableSetupMenu but the command ids are in
 *     ExpertMenuCommands
 */
void
Main::disableExpertMenu() {
  unsigned nCommands = sizeof(ExpertMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuExpert->DisableEntry(ExpertMenuCommands[i]);
  } 
}
void
Main::enableExpertMenu() {
  unsigned nCommands = sizeof(ExpertMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuExpert->EnableEntry(ExpertMenuCommands[i]);
  } 
  
}

/**
 * disable/enableScopeMenu
 *     All the same for the scope menu using the scopeMenuCommands
 *     table.
 */

void
Main::disableScopeMenu() {
  unsigned nCommands = sizeof(ScopeMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuScope->DisableEntry(ScopeMenuCommands[i]);
  } 
}
void
Main::enableScopeMenu() {
  unsigned nCommands = sizeof(ScopeMenuCommands)/sizeof(Commands);
  for (unsigned i = 0; i < nCommands; i++) {
    MenuScope->EnableEntry(ScopeMenuCommands[i]);
  } 
  
}
/**
 * disableButtons
 *    Disables the analyzeB, acquireB and saveB buttons:
 */
void
Main::disableButtons() {
  analyzeB->SetEnabled(kFALSE);
  acquireB->SetEnabled(kFALSE);
  saveB->SetEnabled(kFALSE);
}
void Main::enableButtons() {
  analyzeB->SetEnabled(kTRUE);
  acquireB->SetEnabled(kTRUE);
  saveB->SetEnabled(kTRUE);
  
}
// pMsg below never gets referenced so this bit of pragma magic in gcc
// turns off the warning about that.

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wunused-variable"
void Main::messageBox(const char* txt)
{
  TGMsgBox* pMsg = new TGMsgBox(gClient->GetRoot(), this, "", txt);
}
