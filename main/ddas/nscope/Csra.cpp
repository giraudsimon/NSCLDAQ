...
Csra::load_info (Long_t mod, Bool_t verbose)
{
  ...
  for (int i = 0; i < 16; i++)
    ...
    if (verbose)
        cout << "Channel " << i << " " << std::hex << (uint32_t)ChanParData << std::dec << endl;
    
    // Ensure that 'converting' from double -> uint32 does not corrupt
    // bits;
    
    
    std::bitset<32> csra((uint32_t)ChanParData);
    
    std::cout << "Channel " << i << " value: " << std::hex << csra.to_ulong() << std::dec << std::endl;
    
    // Note the bits are numbered opposite the checkbuttons in std::bitset
...
Csra::change_values (Long_t mod)
{
  ...
  for (int i = 0; i < 16; i++)
    ...
    // Ensure when we stuff ChanParData the bits don't get corrupted.
    
    std::cout << std::hex << " bitset: " << csra.to_ulong() << std::dec << std::endl;
    ChanParData = csra.to_ulong();
      std::cout << std::setw(4) << i << " " 
					<< std::hex << std::showbase   << std::setw(12) << (uint32_t)ChanParData 
